...
def train(
    ...
    # PATH 가 없으면 make directory => 내가 추가한것.
    if not os.path.exists('{}/weights'.format(output_location)):
        os.makedirs('{}/weights'.format(output_location))
    PATH = 'output_location/weights'
    ...
    for ep in range(start_step.ep, end_step.ep + 1):
        ...
        # Before Training parameter 저장 => 내가 추가
        torch.save(model.state_dict(), '{}/weights/Record_Weights_Before_ep{}.pth'.format(output_location, ep))
        ...
        # Path 관련 설정 => 내가 추가
        paths.weights(PATH,ep)

        """
        if not get_platform().is_primary_process: return
        if not get_platform().exists(PATH): get_platform().makedirs(PATH)
        """

        # After Training Parameter 저장 => 내가 추가
        torch.save(model.state_dict(),'{}/weights/Record_Weights_After_ep{}.pth'.format(output_location, ep)) # PARAMETER 저장
